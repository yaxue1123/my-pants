...
<body>
	...
	<div class="container index">
			 ...
			 <form class="pure-form pure-form-aligned" method="post" action="Order?action=add">
		    <div class="orderInfor">
		    <fieldset>
		    <legend style="font-size:16px;">订单详情</legend>
		    	<%
		    	String price=request.getParameter("price");
		    	String userName=request.getParameter("userName");
		    	out.print("<div class='pure-control-group'><label>订单总金额</label>");
		    	out.print("<input class='inputTxt' name='price' readonly='readonly'  value='"+price+"'/></div>");
		    	out.print("<div class='pure-control-group'><label>购买用户</label>");
		    	out.print("<input class='inputTxt' name='userName' readonly='readonly'  value='"+userName+"'/></div>");
		    	Class.forName("com.mysql.jdbc.Driver").newInstance(); 
		    	Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/shop?user=root&password="); 
		    	Statement st = conn.createStatement(); 
		    	ResultSet rs = st.executeQuery("SELECT Password FROM user WHERE LoginName='"+s+"'"); 
		    	String ps= null;
		    	while(rs.next()){
		     	ps=rs.getString(1);}
		    	out.print("<input class='inputTxt' name='passwordR' type='hidden' value='"+ps+"'/>");
		    	%>
		    </fieldset>
		    </div>
		    <div class="orderInfor">
		    <fieldset>
			    <legend style="font-size:16px;">收件人信息</legend>
			        ...
			        <div class="pure-controls">
			            <button type="submit" class="pure-button pure-button-primary">确认提交</button>
			        </div>
		    </fieldset>
		     </div>
		    </form>
