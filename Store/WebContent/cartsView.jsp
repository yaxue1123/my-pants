...
<body>
	...
	<div class="container index">
		...
		<thead>
			<tr><th width="100px">图片</th><th width="250px">商品</th><th width="100px">大小</th><th width="100px">颜色</th><th width="100px">价格</th><th width="100px">数量</th><th width="100px">小计</th><th width="100px">操作</th></tr>
		</thead>		
		<tbody>	
		<c:forEach var="item" items="${requestScope.Carts}">	
			<tr>
				<td class="viewPicture"><img src="pics/pic${item.productId }.png" ></td>				
				...
				<td>${item.sum}</td>											
				<td>
					<form action="Cart?action=del" method="post">
						<input type="hidden" name="id" value="${item.id }" class="pure-button pure-button-primary" />
						<input type="submit" value="删除" class="pure-button pure-button-primary" />
					</form>
				</td>	
			</tr>
		</c:forEach>
		...
		<form action="Cart?action=delAll" method="post">	
			<input type="submit" value="清空购物车" class="pure-button pure-button-primary" />
		</form>
	<div class="totalPrice">
		...
		<% 
		   float n=0;
		    ...
		    Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/shop?user=root&password="); 
		    Statement st = conn.createStatement(); 
		    ResultSet rs = st.executeQuery("SELECT Sum FROM cart"); 
		 
		    while(rs.next()){ 
		    	n=n+rs.getFloat("Sum");
		    } 
			out.print("<input class='inputTxt' name='price' readonly='readonly'  value="+n+">");
		    conn.close(); 	    
		    out.print("<input type='hidden' name='userName' value="+s+">");
		%> 
		<input type="submit" value="提交订单" class="pure-button pure-button-primary" />
		</form>
	</div>
	</div>
</body>
